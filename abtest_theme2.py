...
DATA_FILE = 'ab_test_data_theme2.csv'
...
st.caption("0 = 全く見たくない　／　10 = 今すぐ見たい")
...
if os.path.exists(DATA_FILE):
    ...
    with col2:
        ...
        ax2.set_title("Boxplot of Rating")
        st.pyplot(fig2)
    ...
    st.subheader("平均見たい度の95%信頼区間")
    ci_a = compute_ci(a_data)
    ci_b = compute_ci(b_data)

    if ci_a:
        st.write(
            f"Aの平均見たい度: {a_data.mean():.2f} "
    ...
    if len(a_data) >= 2 and len(b_data) >= 2:
        ...
        st.subheader("平均見たい度のt検定")
        st.write(f"p値 = {p_value:.4f}（"
                 "「サムネAとBで見たい度の平均に差がない」という仮説の下で、"
                 "このデータ以上の差が得られる確率）")
        alpha = 0.05
        if p_value < alpha:
